...
main(argc, argv)
    ...
    char          **argv;
{
    int             i, j, k, ii, jj, kk, l, n, nmax = 0, at1, at2, nequiv, **neqmat;
...
/*     printf("MAXRMSD = %f\n", maxrmsd); */
    ...
    neqmat = allocmat(nmodels, nmodels, sizeof(int), TRUE);
    rmsdmat = allocmat(nmodels, nmodels, sizeof(float), TRUE);
...
// HERE
	    neqmat[moda][modb] = neqmat[modb][moda] = nequiv;
	    rmsdmat[moda][modb] = rmsdmat[modb][moda] = sqrt(rmsd / natoms[0]);
#if 0
	    if (nequiv >= 40)
		rmsdmat[moda][modb] = rmsdmat[modb][moda] = maxrmsd;
#endif
	}
    ...
    {
	for (cutoff=minrmsd; cutoff<=maxrmsd; cutoff+=0.5)
	{
	    maxclusz = repnum = 0;
	    for (moda=0; moda<nmodels; moda++)
		if (!clustflg[moda])
		{
			...
			if (modb != moda)
			{
			    if (rmsdmat[moda][modb] <= cutoff && !clustflg[modb])
				nmatch[moda]++;
			    if (nmatch[moda] > maxclusz)
			    {
				maxclusz = nmatch[moda];
				repnum = moda;
			    }
			}
		}
  ...
  ofp = fopen(fname, "w");
	...
	if (ofp != NULL)
	{
	    ...
	    for (modb=moda=0; moda<nmodels; moda++)
